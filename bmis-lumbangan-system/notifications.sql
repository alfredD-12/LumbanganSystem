...
FOR EACH ROW
BEGIN
    -- Notify the complainant (if registered) and officials
    IF NEW.user_id IS NOT NULL THEN
        INSERT INTO `notifications` 
        (`user_id`, `user_type`, `notification_type`, `title`, `message`, `link`, `reference_id`)
        VALUES 
        (NEW.user_id, 'user', 'complaint', 
         CONCAT('Complaint Submitted: ', NEW.incident_title),
         CONCAT('Your complaint "', NEW.incident_title, '" has been submitted and is now pending review.'),
         CONCAT('?page=resident_complaints#incident-', NEW.id),
         NEW.id);
    END IF;

    -- Notify officials (admins handle complaints)
    INSERT INTO `notifications` 
    (`user_id`, `user_type`, `notification_type`, `title`, `message`, `link`, `reference_id`)
    VALUES 
...
BEGIN
    ...
    IF OLD.status_id <> NEW.status_id THEN
        ...
        -- Notify the specific resident/user if user_id is available
        IF NEW.user_id IS NOT NULL THEN
            INSERT INTO `notifications` 
            (`user_id`, `user_type`, `notification_type`, `title`, `message`, `link`, `reference_id`)
            VALUES 
            (NEW.user_id, 'user', 'complaint', 
             CONCAT('Your Complaint: ', NEW.incident_title),
             CONCAT('Status updated to: ', status_text),
             CONCAT('?page=resident_complaints#incident-', NEW.id),
             NEW.id);
        END IF;

        -- Also notify officials about status change
        INSERT INTO `notifications` 
        (`user_id`, `user_type`, `notification_type`, `title`, `message`, `link`, `reference_id`)
        VALUES 
        (NULL, 'official', 'complaint', 

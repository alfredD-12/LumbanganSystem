...
<body>
  ...
  <script>
    ...
    const forgetCaptchaRequired = {
      1: false,
      2: false,
      3: false
    };

    function getForgetAuthConfig() {
      return {
        csrfField: document.querySelector('meta[name="csrf-field"]')?.content || 'csrf_token',
        csrfToken: document.querySelector('meta[name="csrf-token"]')?.content || '',
        recaptchaSiteKey: document.querySelector('meta[name="recaptcha-site-key"]')?.content || '',
      };
    }

    function resetForgetCaptcha(step, hideContainer = false) {
      if (window.BMISCaptcha && typeof window.BMISCaptcha.reset === 'function') {
        window.BMISCaptcha.reset(`forget-step${step}`);
      }

      const hiddenInput = document.getElementById(`forgetCaptchaTokenStep${step}`);
      if (hiddenInput) {
        hiddenInput.value = '';
      }

      if (hideContainer) {
        const container = document.getElementById(`forgetRecaptchaContainer${step}`);
        if (container) {
    ...
    function clearForgetFeedback(step) {
      const errorDiv = document.getElementById(`forgetStep${step}Error`);
      const successDiv = step === 3 ? document.getElementById('forgetStep3Success') : null;

      if (errorDiv) {
        errorDiv.style.display = 'none';
        const countdown = errorDiv.querySelector('.forget-retry-countdown');
        if (countdown) {
          countdown.remove();
        }
      }
    ...
    function showForgetStepError(step, message, code = '', retryAfter = 0) {
      const errorDiv = document.getElementById(`forgetStep${step}Error`);
      ...
      const existingCountdown = errorDiv.querySelector('.forget-retry-countdown');
      ...
      if (retryAfter > 0) {
        ...
        countdown.className = 'forget-retry-countdown';
        countdown.style.display = 'block';
        countdown.style.marginTop = '6px';
        errorDiv.appendChild(countdown);

        if (typeof startRetryCountdown === 'function') {
          startRetryCountdown(countdown, retryAfter);
        } else {
          countdown.textContent = `Please wait ${retryAfter} seconds before retrying.`;
        }
      }

      errorDiv.style.color = code === 'captcha_required' || code === 'rate_limit_exceeded' ? '#9a6700' : '#d32f2f';
      errorDiv.style.display = 'block';
    }

    function resetForgetWorkflow() {
      forgetPasswordEmailNew = '';
      forgetPasswordTokenNew = '';
      forgetCaptchaRequired[1] = false;
      forgetCaptchaRequired[2] = false;
      forgetCaptchaRequired[3] = false;
      ...
      clearForgetFeedback(1);
      clearForgetFeedback(2);
      clearForgetFeedback(3);

      resetForgetCaptcha(1, true);
      resetForgetCaptcha(2, true);
      resetForgetCaptcha(3, true);

      showForgetStep(1);
    }

    async function ensureForgetCaptcha(step) {
      const {
        recaptchaSiteKey
      } = getForgetAuthConfig();

      forgetCaptchaRequired[step] = true;

      if (!recaptchaSiteKey || !window.BMISCaptcha || typeof window.BMISCaptcha.ensureVisibleCaptcha !== 'function') {
        return false;
      }

      const rendered = await window.BMISCaptcha.ensureVisibleCaptcha(
        `forget-step${step}`,
        `forgetRecaptchaContainer${step}`,
        `forgetRecaptchaWidget${step}`,
        recaptchaSiteKey
      );
    ...
    function buildForgetFormData(step) {
      const {
        csrfField,
        csrfToken
      } = getForgetAuthConfig();
      ...
      if (forgetCaptchaRequired[step]) {
        const captchaToken = window.BMISCaptcha && typeof window.BMISCaptcha.getToken === 'function'
          ? window.BMISCaptcha.getToken(`forget-step${step}`)
          : '';

        if (!captchaToken) {
          showForgetStepError(step, 'Please complete the reCAPTCHA challenge before continuing.');
          return null;
        }
    ...
    async function handleForgetError(step, data, fallbackMessage) {
      ...
      if (code === 'captcha_required') {
        const rendered = await ensureForgetCaptcha(step);
        message = rendered
          ? 'Too many attempts. Please complete reCAPTCHA and try again.'
          : 'Unable to load reCAPTCHA challenge. Please refresh and try again.';
      }

      showForgetStepError(step, message, code, Number(data?.retry_after || 0));

      if (forgetCaptchaRequired[step]) {
        resetForgetCaptcha(step, false);
      }
    }
    ...
    async function submitForgetEmailNew(e) {
      e.preventDefault();
      clearForgetFeedback(1);

      const email = document.getElementById('forgetEmail').value.trim();
      if (!email) {
        showForgetStepError(1, 'Please enter your email.');
        return;
      }

      const formData = buildForgetFormData(1);
      if (!formData) {
        return;
      }
      ...
      try {
        const response = await fetch('<?php echo (defined('BASE_PUBLIC') ? rtrim(BASE_PUBLIC, '/') : '') . '/index.php'; ?>?action=request_reset', {
          method: 'POST',
          body: formData,
        ...
        await handleForgetError(1, data, 'Failed to send reset code.');
      } catch (error) {
        showForgetStepError(1, 'An error occurred. Please try again.');
        console.error('Password reset request error:', error);
      }
    }

    async function submitForgetCodeNew(e) {
      e.preventDefault();
      clearForgetFeedback(2);

      const code = document.getElementById('forgetCode').value.trim();
      if (!code || code.length !== 6) {
        showForgetStepError(2, 'Please enter a valid 6-digit code.');
        return;
      }

      const formData = buildForgetFormData(2);
      if (!formData) {
        return;
      }
      ...
      try {
        ...
        await handleForgetError(2, data, 'Invalid or expired code.');
      } catch (error) {
        showForgetStepError(2, 'An error occurred. Please try again.');
        console.error('Password reset verify error:', error);
      }
    }

    async function submitResetPasswordNew(e) {
      e.preventDefault();
      clearForgetFeedback(3);
      ...
      if (!password || !confirmPassword) {
        showForgetStepError(3, 'Please enter both passwords.');
        return;
      }

      if (password.length < 6) {
        showForgetStepError(3, 'Password must be at least 6 characters.');
        return;
      }

      if (password !== confirmPassword) {
        showForgetStepError(3, 'Passwords do not match.');
        return;
      }

      const formData = buildForgetFormData(3);
      if (!formData) {
        return;
      }
      ...
      try {
        const response = await fetch('<?php echo (defined('BASE_PUBLIC') ? rtrim(BASE_PUBLIC, '/') : '') . '/index.php'; ?>?action=reset_password', {
          method: 'POST',
          body: formData,
          credentials: 'same-origin'
        });
        ...
        if (data.success) {
          ...
          if (successMsg) {
            successMsg.textContent = data.message || 'Password reset successfully!';
          }
          if (successDiv) {
        ...
        await handleForgetError(3, data, 'Failed to reset password.');
      } catch (error) {
        showForgetStepError(3, 'An error occurred. Please try again.');
        ...
        <div id="forgetStep1" style="display: block;">
          ...
          <form onsubmit="submitForgetEmailNew(event)" style="display: flex; flex-direction: column;">
            ...
            <input type="email" placeholder="Email address" id="forgetEmail" required style="
              ...
              width: 100%;
            ">
            <input type="hidden" id="forgetCaptchaTokenStep1" value="">
            <div id="forgetRecaptchaContainer1" style="display: none; margin-bottom: 15px;">
              <div id="forgetRecaptchaWidget1"></div>
            </div>
        ...
        <div id="forgetStep2" style="display: none;">
          ...
          <form onsubmit="submitForgetCodeNew(event)" style="display: flex; flex-direction: column;">
            ...
            <input type="text" placeholder="000000" maxlength="6" id="forgetCode" required style="
              ...
              width: 100%;
            ">
            <input type="hidden" id="forgetCaptchaTokenStep2" value="">
            <div id="forgetRecaptchaContainer2" style="display: none; margin-bottom: 15px;">
              <div id="forgetRecaptchaWidget2"></div>
            </div>
        ...
        <div id="forgetStep3" style="display: none;">
          ...
          <form onsubmit="submitResetPasswordNew(event)" style="display: flex; flex-direction: column;">
            ...
            <input type="password" placeholder="Confirm Password" id="forgetConfirmPassword" required style="
              ...
              width: 100%;
            ">
            <input type="hidden" id="forgetCaptchaTokenStep3" value="">
            <div id="forgetRecaptchaContainer3" style="display: none; margin-bottom: 15px;">
              <div id="forgetRecaptchaWidget3"></div>
            </div>
